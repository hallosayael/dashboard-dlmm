...
// Ringkas ala Indonesia untuk IDR: rb (ribu), jt (juta), M (miliar), T (triliun).
// Desimal pakai koma. >=100 di satuannya dibulatkan ke bulat biar pendek.
function shortAbsId(a) {
  const f = (x, suf) => {
    const r = x >= 100 ? String(Math.round(x)) : String(Math.round(x * 10) / 10);
    return r.replace('.', ',') + suf;
  };
  if (a >= 1e12) return f(a / 1e12, 'T');
  if (a >= 1e9) return f(a / 1e9, 'M');
  if (a >= 1e6) return f(a / 1e6, 'jt');
  if (a >= 1e3) return f(a / 1e3, 'rb');
...
export function fmtMoney(sol, cur, solUsd, usdIdr, opts = {}) {
  ...
  if (cur === 'idr') {
    const body = compact ? shortAbsId(a) : Math.round(a).toLocaleString('id-ID');
