...
// Ambang "impas" (breakeven): kalau |pnl| DI BAWAH 0.0001 SOL dianggap impas
// (bukan menang bukan kalah). Mulai 0.0001 ke atas/bawah baru untung/rugi.
// Contoh: +0.0002 win · +0.0001 win · -0.0000 impas · -0.0004 loss.
export const BREAKEVEN_EPS = 1e-4; // 0.0001 SOL

// 1 = profit, 0 = impas, -1 = loss
export function pnlState(sol) {
  const n = Number(sol);
  if (!Number.isFinite(n)) return 0;
  if (Math.abs(n) < BREAKEVEN_EPS) return 0; // impas: di bawah 0.0001 SOL
  return n > 0 ? 1 : -1;
}
