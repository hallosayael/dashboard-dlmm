...
// Ambang "impas" (breakeven): |pnl| di bawah ini dianggap membulat ke nol,
// jadi bukan menang bukan kalah — supaya minus/plus tipis tidak dicap loss/profit.
export const BREAKEVEN_EPS = 5e-4; // 0.0005 SOL

// 1 = profit, 0 = impas, -1 = loss
export function pnlState(sol) {
  const n = Number(sol);
  if (!Number.isFinite(n)) return 0;
  if (n > BREAKEVEN_EPS) return 1;
  if (n < -BREAKEVEN_EPS) return -1;
  return 0;
}
