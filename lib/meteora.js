...
export async function getWalletData(walletInput) {
  ...
  const [positions, solPrice] = await Promise.all([getClosedPositions(wallet), getSolUsd()]);
  return { wallet, solPrice, demo: false, positions };
}
...
function buildDemo() {
  ...
  return { wallet: 'demo', solPrice: FALLBACK_SOL_PRICE, demo: true, positions };
}
