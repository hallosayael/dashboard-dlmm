...
async function getPoolList(wallet) {
  ...
  for (let i = 0; i < MAX_LIST_PAGES; i++) {
    ...
    for (const p of j?.pools || []) {
      // logo token dasar (yang bukan SOL) untuk marker
      const icon = p.tokenX === 'SOL' ? p.tokenYIcon : p.tokenXIcon;
      pools.push({
        poolAddress: p.poolAddress,
        pair: `${p.tokenX || '?'}/${p.tokenY || '?'}`,
        binStep: p.binStep,
        baseFee: p.baseFee,
        icon: icon || p.tokenXIcon || '',
      });
    }
    if (!j?.hasNext) break;
    page++;
...
export async function getClosedPositions(wallet) {
  ...
  // tandai pemilik posisi — dipakai compare & ringkasan per-wallet
  for (const p of positions) p.owner = wallet;
