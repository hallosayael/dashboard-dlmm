...
// warna huruf mengikuti kelas CSS yang sudah ada: gr / am / rd
...
export function computeHealth(positions) {
  ...
  const { maxDD, peak } = drawdown(positions);
  const ddDenom = Math.max(peak, Math.abs(totalPnl), 0.05);
  const relDD = maxDD / ddDenom;

  // sub-skor (rumus heuristik, transparan)
  const profScore = clamp(50 + roiAgg * 1000, 0, 100);  // ROI agregat vs modal
  const consScore = clamp(winRate * 100, 0, 100);        // winrate
  const effScore = clamp(100 - ilRatio * 70, 0, 100);    // seberapa fee terjaga dari IL
  const riskScore = clamp(100 - sd * 14, 0, 100);        // volatilitas antar-posisi
  const ddScore = clamp(100 - relDD * 120, 0, 100);      // kedalaman drawdown
  ...
  // insight (rule-based)
  const insights = [];
  if (ilRatio >= 0.4) insights.push(`IL memakan ${Math.round(ilRatio * 100)}% fee`);
  ...
  if (long.length >= 3 && short.length >= 3 && mean(long.map((p) => p.pnlPct || 0)) < mean(short.map((p) => p.pnlPct || 0)) - 0.3) {
    insights.push('hold >2 jam kurang optimal');
  }
  if (winRate < 0.5) insights.push(`winrate ${Math.round(winRate * 100)}% — di bawah 50%`);
  if (relDD > 0.35) insights.push(`drawdown cukup dalam (${Math.round(relDD * 100)}%)`);
  if (!insights.length) insights.push('metrik dalam batas sehat');

  return { n, overall, status, grades, insights: insights.slice(0, 3), winRate, roiAgg, ilRatio };
}
...
export function computeInsight(positions) {
  ...
  const short = hp.filter((p) => (p.closedAt - p.createdAt) < 7200);
  const long = hp.filter((p) => (p.closedAt - p.createdAt) >= 7200);
  const longWorse = long.length >= 3 && short.length >= 3
    && mean(long.map((p) => p.pnlPct || 0)) < mean(short.map((p) => p.pnlPct || 0));
