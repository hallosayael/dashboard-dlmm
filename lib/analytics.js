...
export function computeHealth(positions) {
  ...
  const winRate = (wins + losses) ? wins / (wins + losses) : 0; // impas dikecualikan
...
export function computePools(positions) {
  ...
  for (const p of positions) {
    ...
    if (st > 0) o.wins += 1; else if (st < 0) o.losses = (o.losses || 0) + 1;
  }
  return [...map.values()]
    .map((o) => ({ ...o, losses: o.losses || 0, winRate: (o.wins + (o.losses || 0)) ? o.wins / (o.wins + (o.losses || 0)) : 0 }))
...
export function computeAudit(positions) {
  ...
  const state = pnlState(growth); // 1 pass / 0 impas / -1 review
  const note = state > 0 ? 'consistent — hasil net positif'
    : state < 0 ? 'review — hasil net negatif'
      : 'impas — hasil net ~0';
  return {
    n: positions.length, deposit, withdraw, fees, priceImpact, growth,
    pass: state >= 0, state, note,
  };
}
...
export function computeCompare(positions, wallets) {
  ...
  const map = new Map(list.map((w) => [w, { wallet: w, pnl: 0, n: 0, wins: 0, losses: 0 }]));
  for (const p of positions) {
    ...
    o.n += 1;
    const st = pnlState(p.pnlSol);
    if (st > 0) o.wins += 1; else if (st < 0) o.losses += 1;
  }
  const rows = [...map.values()]
    .map((o) => ({ ...o, winRate: (o.wins + o.losses) ? o.wins / (o.wins + o.losses) : 0 }))
    .sort((a, b) => b.pnl - a.pnl);
  ...
  return { rows, combined, winRate: (totalW + totalL) ? totalW / (totalW + totalL) : 0, curves, single: list.length < 2 };
}
